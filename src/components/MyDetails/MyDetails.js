...
const MyDetails = () => {
    const { mydetailsId } = useParams()
    const [mySingleProject, setMySingelProjects] = useState([])
    ...
    useEffect(() => {
        ...
        fetch(url)
            .then(res => res.json())
            .then(data => {
                let searchId = data.filter((singledata) => singledata.id === mydetailsId);
                setMySingelProjects(searchId)
    ...
    return (
        <div>
            {
                mySingleProject.map(singleService => (
                    <div>
